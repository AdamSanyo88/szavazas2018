    ...
    $.ajax({
		...
		for ( var i = 0; i < lines.length; i++ ) {
			if ( lines[i][0] == window.location.href.substring(window.location.href.lastIndexOf('/') + 1) ) {
				for ( var j=0; j<3; j++ )
					cdata.push([lines[i][20+(j*3)],parseFloat(parseFloat(lines[i][22+(j*3)]).toFixed(2)),lines[i][21+(j*3)],(lines[i][22+(j*3)]*100)+"%"]);
				...
				for ( var j=0; j<8; j++ )
					cdata2.push([ogyparts[j],parseFloat(parseFloat(lines[i][2+(j*3)]).toFixed(2)),ogycolors[j],(lines[i][2+(j*3)]*100)+"%"]);
				console.log(cdata2)
				break;
			}
		}
